...
<html>
  ...
  <div><h4>${thisJoinMatchTeamPeople.name }的“蓝翔二号”手掷飞机直线距离赛 打分表</h4></div>
    ...
    <input type="hidden" value="${thisJoinMatchTeamPeople.peoplesn}" name="peoplesn" >
    <input type="hidden" value="${thisJoinMatchTeamPeople.teamname}" name="jointeamname" >
    <input type="hidden"  value="${thisJoinMatchTeamPeople.name}" name="peoplename" >
    ...
    <c:choose>
    <c:when test="${thisPeopleScore.score211 eq null}"><input type="text" name="score211" ></c:when>
    <c:when test="${!(thisPeopleScore.score211 eq null)}">${thisPeopleScore.score211}</c:when>
    </c:choose>
    </td>
    <td>
  <c:choose>
   ...
   <c:when test="${!(matchlist1.score211 eq null)}"><input type="text" name="score212"></c:when>
   <c:when test="${(matchlist1.score211 eq null)}"><input type="text" name="score212"></c:when>
   ...
   <!--<td><input type="text" name="score3"></td>-->
    ...
    <button type="submit" onclick="getMarking21()">确定打分</button>
    </form>
    </div>
   
    <script type="text/javascript">
    function getMarking21(){
    	var form =document.forms[0];
    	form.action="<%=basePath%>getMarking21";
    	form.method="post";
    	form.submit();}  
    </script>
  </body>
