...
<html>
  ...
  <body>
  <div align="center">
  <div><h4>${thisJoinMatchTeamPeople.name }的“钻石 300”手掷滑翔机三人接力赛 打分表</h4></div>
    <form action="" >
   ...
   <input type="text" name="scoreone" style="width:75px">圈数（例：4+A、3+b）
    ...
    <td>第二轮成绩</td>
    <td>
 <input type="text" name="scoretwo" style="width:75px">圈数（例：4+A、3+b）
   </td>
    </tr>
   ...
   <!--<td><input type="text" name="score3"></td>-->
    ...
    <c:choose>
   <c:when test="${(thisPeopleScore.score82 eq null)|| thisPeopleScore.score82 eq '0.0'}"> <button type="submit" onclick="getMarking4()">确定打分</button></c:when>
   <c:when test="${!(thisPeopleScore.score82 eq '0.0')&&!(thisPeopleScore.score82 eq null)}"> <a href="readAndMarking?refereeid=${rid}">扫码打分</a></c:when>
   </c:choose>
    ...
    function getMarking4(){
    	var form =document.forms[0];
    	form.action="<%=basePath%>getMarking4";
    	form.method="post";
    	form.submit();}  
    </script>
  </body>
