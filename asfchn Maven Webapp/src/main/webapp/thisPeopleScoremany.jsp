...
<html>
  ...
  <div align="center">
  <div><h1>打分成功</h1></div> 
  <div><h4>${mangoptionsmatch.name }的第一轮成绩为${ mangoptionsmatch.scorefinalone} ，第二轮成绩为${ mangoptionsmatch.scorefinaltwo}  ,总成绩为${mangoptionsmatch.scorefinal }</h4></div> 
  <a href="startMarking?id=${joinMatchTeamPeople.id}&rid=${mangoptionsmatch.rid}">觉得分数有误？点此返回修改</a>
  
  <div>
    <p>请将扫码设备对准<span style="font-weight: bold">${mangoptionsmatch.name }</span>的二维码</p>
    <form action="">
    <input type="hidden" name="joinpeopleid" value="${mangoptionsmatch.joinpeopleid}">
    <input type="hidden" name="peopleid" value="${hxpeople.id}">
     <input type="hidden" name="rid" value="${mangoptionsmatch.rid }">
     <input type="hidden" name="manyid" value="${mangoptionsmatch.id }">
