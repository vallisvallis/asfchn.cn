...
<html>
  ...
  <div align="center">
  <div><h1>打分成功</h1></div> 
  <div><h4>${withfriendmatch.name }的第一轮成绩为${ withfriendmatch.scoreOne} ，第二轮成绩为${ withfriendmatch.scoretwo}  ,总成绩为${withfriendmatch.finalscore }</h4></div> 
  <a href="startMarking?id=${withfriendmatch.joinpeopleid}&rid=${withfriendmatch.rid}">觉得分数有误？点此返回修改</a>
  
  <div>
    <p>请将扫码设备对准<span style="font-weight: bold">${withfriendmatch.name }</span>的二维码</p>
    <form action="">
    <input type="hidden" name="joinpeopleid" value="${withfriendmatch.joinpeopleid}">
     <input type="hidden" name="peopleid" value="${hxpeople.id }">
     <input type="hidden" name="rid" value="${withfriendmatch.rid }">
     <input type="hidden" name="twoid" value="${withfriendmatch.id }">
