...
function topayYearFeeForPeopleAgain(){
	var form = document.forms[0];
	form.action = "<%=basePath%>topayYearFeeForPeopleAgain";
	form.method="post";
	form.submit();
	}	
  ...
  <body>
  <div align="center">
  <h3>${hxTeam.fullname}中未交年费的会员</h3>
   <div class="table-responsive" >
   <form name="toPayForPeople" action="" class="form-horizontal" >
							...
							<table class="table table-striped table-hover" id="ListTB">
								...
								<tbody>
									<c:forEach items="${hxPeopleList}" var="hxPeople">
										<tr>
										<th> <input name="peopleList" type="checkbox" value="${hxPeople.id }" /> </th>
											<td>${hxPeople.name }</td>
											<td>${hxPeople.sn }</td>
		...
		<div >
			<button type="submit" class="btn btn-default" onclick="topayYearFeeForPeopleAgain()" align="center">准备缴费</button>
		</div>
	</div>
							</form>
						</div>
