...
<html>
  ...
  <div align="center">
  <div><h1>打分成功</h1></div> 
  <div><h4>${manyrifmatch.name }成绩已记录</h4></div> 
 <!--   <a href="startMarking?id=${joinMatchTeamPeople}&rid=${manyrifmatch.rid}">觉得分数有误？点此返回修改</a>-->
  
  <div>
    <p>请将扫码设备对准<span style="font-weight: bold">${manyrifmatch.name }</span>的二维码</p>
    <form action="">
    <input type="hidden" name="joinpeopleid" value="${manyrifmatch.joinpeopleid}">
     <input type="hidden" name="peopleid" value="${hxpeople.id }">
     <input type="hidden" name="rid" value="${manyrifmatch.rid }">
     <input type="hidden" name="manyid" value="${manyrifmatch.id }">
